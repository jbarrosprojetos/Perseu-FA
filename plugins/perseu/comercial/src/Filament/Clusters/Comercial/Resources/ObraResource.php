...
class ObraResource extends Resource
{
    ...
    {
        return __('comercial::filament/resources/obra.model-label');
    }

    public static function getPluralModelLabel(): string
    {
        return __('comercial::filament/resources/obra.plural-model-label');
    }

    public static function getNavigationLabel(): string
    {
        return __('comercial::filament/resources/obra.navigation.title');
    }
    ...
    public static function form(Schema $schema): Schema
    {
        ...
        // columnSpan antes deles (numero_obra+revisao+data_cadastro = 3
        ...
        return $schema
            ->components([
                // Linha 1: numero_obra/revisao/data_cadastro (1 coluna
                // cada) + descricao "Nome da Obra" (4) + tipo_obra_id (2)
                ...
                Grid::make(12)
                    ...
                    ->schema([
                        ...
                        Placeholder::make('numero_obra')
                            ->label(__('comercial::filament/resources/obra.form.numero-obra'))
                            ->content(fn (?Obra $record) => $record?->numero_obra
                                ?? __('comercial::filament/resources/obra.form.numero-obra-pendente'))
                            ->extraAttributes(['class' => 'fi-entry-bold'])
                            ->columnSpan(1),
                        Placeholder::make('revisao_display')
                            ->label(__('comercial::filament/resources/obra.form.revisao'))
                            ->content(fn (?Obra $record) => str_pad((string) ($record->revisao ?? 0), 2, '0', STR_PAD_LEFT))
                            ->extraAttributes(['class' => 'fi-entry-bold'])
                            ->columnSpan(1),
                        Placeholder::make('data_cadastro')
                            ->label(__('comercial::filament/resources/obra.form.data-cadastro'))
                            ->content(fn (?Obra $record) => $record?->data_cadastro?->format('d/m/Y')
                                ?? __('comercial::filament/resources/obra.form.data-cadastro-pendente'))
                            ->extraAttributes(['class' => 'fi-entry-bold'])
                            ->columnSpan(1),
                        TextInput::make('descricao')
                            ->label(__('comercial::filament/resources/obra.form.descricao'))
                            ->required()
                            ->maxLength(255)
                            ->columnSpan(4),
                        Select::make('tipo_obra_id')
                            ->label(__('comercial::filament/resources/obra.form.tipo-obra'))
                            ->relationship('tipoObra', 'descricao')
                            ->required()
                            ->searchable()
                            ->preload()
                            ->columnSpan(2),
                        Select::make('situacoes')
                            ->label(__('comercial::filament/resources/obra.form.situacoes'))
                ...
                Grid::make(12)
                    ...
                    ->schema([
                        Radio::make('tipo_contratante')
                            ->label(__('comercial::filament/resources/obra.form.tipo-contratante'))
                            ->options([
                                'pf' => __('comercial::filament/resources/obra.form.tipo-contratante-options.pessoa-fisica'),
                                'pj' => __('comercial::filament/resources/obra.form.tipo-contratante-options.pessoa-juridica'),
                            ])
                            ->inline()
                            ->live()
                            ->dehydrated(false)
                        ...
                        Select::make('pessoa_fisica_id')
                            ->label(__('comercial::filament/resources/obra.form.pessoa-fisica'))
                            ->relationship(
                                name: 'pessoaFisica',
                                titleAttribute: 'nome',
                        ...
                        Select::make('pessoa_juridica_id')
                            ->label(__('comercial::filament/resources/obra.form.pessoa-juridica'))
                            ->relationship(
                                name: 'pessoaJuridica',
                        ...
                        Select::make('contato_pessoa_fisica_id')
                            ->label(__('comercial::filament/resources/obra.form.contato'))
                        ...
                        Placeholder::make('contato_email')
                            ->label(__('comercial::filament/resources/obra.form.contato-email'))
                        ...
                        Placeholder::make('contato_telefone')
                            ->label(__('comercial::filament/resources/obra.form.contato-telefone'))
                ...
                Select::make('endereco_id')
                    ->label(__('comercial::filament/resources/obra.form.endereco'))
                    ->options(function (Get $get): array {
                    ...
                    ->createOptionForm([
                        TextInput::make('cep')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.cep'))
                            ->mask('99999-999')
                            ->live(onBlur: true)
                            ->afterStateUpdated(fn (Set $set, ?string $state) => ViaCepLookup::fill($set, $state)),
                        TextInput::make('logradouro')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.logradouro')),
                        TextInput::make('numero')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.numero')),
                        TextInput::make('complemento')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.complemento')),
                        TextInput::make('bairro')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.bairro')),
                        TextInput::make('municipio')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.municipio')),
                        TextInput::make('uf')
                            ->label(__('comercial::filament/resources/obra.form.endereco-form.uf'))
                            ->maxLength(2),
                    ])
                    ->createOptionUsing(function (array $data, Get $get): int {
                        ...
                        // tipo mais coerente com o contexto (endereço de Obra).
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->with(['pessoaFisica', 'pessoaJuridica', 'tipoObra', 'situacoes']))
            ->columns([
                TextColumn::make('numero_obra')
                    ->label(__('comercial::filament/resources/obra.table.columns.numero-obra'))
                    ->searchable()
                    ->sortable(),
                TextColumn::make('descricao')
                    ->label(__('comercial::filament/resources/obra.table.columns.descricao'))
                    ->searchable(),
                TextColumn::make('tipoObra.descricao')
                    ->label(__('comercial::filament/resources/obra.table.columns.tipo-obra')),
                TextColumn::make('contratante')
                    ->label(__('comercial::filament/resources/obra.table.columns.contratante'))
                    ->getStateUsing(fn (Obra $record) => $record->pessoaFisica?->nome ?? $record->pessoaJuridica?->nome_fantasia),
                TextColumn::make('situacoes.descricao')
                    ->label(__('comercial::filament/resources/obra.table.columns.situacoes'))
                    ->badge(),
                TextColumn::make('data_cadastro')
                    ->label(__('comercial::filament/resources/obra.table.columns.data-cadastro'))
                    ->dateTime()
                    ->sortable()
            ...
            ->filters([
                TrashedFilter::make()
                    ->label(__('comercial::filament/resources/obra.table.filters.trashed')),
            ])
            ->recordActions([
                EditAction::make()
                    ->successNotification(
                        Notification::make()
                            ->success()
                            ->title(__('comercial::filament/resources/obra.table.actions.edit.notification.title'))
                            ->body(__('comercial::filament/resources/obra.table.actions.edit.notification.body')),
                    ),
                DeleteAction::make()
                    ->successNotification(
                        Notification::make()
                            ->success()
                            ->title(__('comercial::filament/resources/obra.table.actions.delete.notification.title'))
                            ->body(__('comercial::filament/resources/obra.table.actions.delete.notification.body')),
                    ),
                RestoreAction::make()
                    ->successNotification(
                        Notification::make()
                            ->success()
                            ->title(__('comercial::filament/resources/obra.table.actions.restore.notification.title'))
                            ->body(__('comercial::filament/resources/obra.table.actions.restore.notification.body')),
                    ),
                ForceDeleteAction::make()
                    ->successNotification(
                        Notification::make()
                            ->success()
                            ->title(__('comercial::filament/resources/obra.table.actions.force-delete.notification.title'))
                            ->body(__('comercial::filament/resources/obra.table.actions.force-delete.notification.body')),
                    ),
            ])
            ->toolbarActions([
